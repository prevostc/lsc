...
#
# Usage: scripts/review-bytecode.sh [Counter|Token|Vault|Cpamm|WETH|all]
set -euo pipefail
cd "$(dirname "$0")/.."
export PATH="${HOME}/.foundry/bin:${HOME}/.bifrost/bin:${PATH}"

CONTRACTS=(Counter Token Vault Cpamm WETH)
TARGET="${1:-all}"
...
case "$TARGET" in
  Counter|Token|Vault|Cpamm|WETH) review_one "$TARGET" ;;
  all)
    for c in "${CONTRACTS[@]}"; do review_one "$c"; done
    ;;
  *)
    echo "Usage: $0 [Counter|Token|Vault|Cpamm|WETH|all]" >&2
    exit 1
    ;;
esac
