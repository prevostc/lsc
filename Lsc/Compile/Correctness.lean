...
open Lsc.Compile.IR (IRState evalExpr evalStmt evalStmt_seq evalStmt_letBind evalStmt_ifRevertSelector
...
theorem Wei_lower_addCheckedNatStorage_shape :
    Wei.lowerLetBind counterConfig.storage.fieldSlot "n"
        (Wei.addCheckedNatStorage "number" 1) 0 =
      .ok (Wei.incrementLetIR 0) := rfl
...
theorem incrementLet_ir_binds_incremented_local :
    let st := evalStmt { slots := [(0, 5)] } (Wei.incrementLetIR 0)
...
theorem incrementLet_ir_binds_symbolic (v : Nat) :
    (evalStmt (IRState.fromSlots [(0, v)]) (Wei.incrementLetIR 0)).lookupLocal "n" = v + 1 := by
  ...
  have hs3 : evalStmt st2 (.ifRevertSelector (.lt (.local "n") (.local "lsc_n_old")) 0) = st2 := by
    simp [evalStmt_ifRevertSelector, h4]
  have h5 : evalStmt (IRState.fromSlots [(0, v)]) (Wei.incrementLetIR 0) = st2 := by
    change evalStmt st0 (Wei.incrementLetIR 0) = st2
...
theorem incrementLet_lower_preserves_n (v : Nat) :
    (evalStmt (IRState.fromSlots [(0, v)]) (Wei.incrementLetIR 0)).lookupLocal "n" = v + 1 :=
