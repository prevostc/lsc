...
elab_rules : command
  | `(state! $mod:ident where $fieldSyns:stateField*) => do
    ...
    for f in flds do
      ...
      elabCommand (← `(@[simp] def $n : LscV1.Field $stIdent $t := ⟨$off⟩))
    elabCommand (← `(end $fieldsNS))

    -- 3. Build view body (used in instance + @[simp] unfolding theorem)
    let viewArgs ← flds.mapM fun (f : StField) => do
      let off := Syntax.mkNumLit (toString f.offset)
      `(LscV1.FromWord.fromWord (LscV1.World.getStorage w LscV1.defaultSelf $off:num))
    ...
    let embedExpr ← flds.foldlM (init := ← `(w)) fun acc (f : StField) => do
      ...
      `(LscV1.World.setStorage $acc LscV1.defaultSelf $off:num
          (LscV1.ToWord.toWord ($accessor s)))

    -- 5. ContractState instance (bodies inlined — no private intermediary defs)
    elabCommand (← `(instance : LscV1.ContractState $stIdent:ident where
      self  := LscV1.defaultSelf
      view  := fun w => $viewBody
    ...
    elabCommand (← `(@[simp] theorem $selfThmId :
        LscV1.ContractState.self (S := $stIdent:ident) = LscV1.defaultSelf := rfl))
    ...
    elabCommand (← `(@[simp] theorem $viewThmId (w : LscV1.World) :
        LscV1.ContractState.view (S := $stIdent:ident) w = $viewBody := rfl))
    elabCommand (← `(@[simp] theorem $embedThmId (s : $stIdent:ident) (w : LscV1.World) :
        LscV1.ContractState.embed s w = $embedExpr := rfl))
...
elab_rules : command
  | `(contract! $mod:ident $err:ident) => do
    ...
    elabCommand (← `(abbrev $mod:ident (α : Type) :=
      LscV1.ContractM $err:ident $stIdent:ident α))
    elabCommand (← `(namespace $modNS))
    elabCommand (← `(@[simp] def $viewId : LscV1.World → $stIdent:ident :=
      LscV1.ContractState.view))
