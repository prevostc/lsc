...
| 2 Core→Yul | `toYulFn_correct_ext`, `runtimeBlock_correct_ext`; instances `vault_correct_ext`, `amm_correct_ext` | `Proof/CoreExtSim.lean`, `Proof/DispatchExt.lean`, `Proof/Vault.lean`, `Proof/Amm.lean` | proved | S2 | Vault (one binding); AMM (two `IERC20`) |
...
| 4 Glue | `bytecode_call_correct_ext`, `yul_progress` | `EndToEndExt.lean`, `Proof/ProgressCore.lean` | proved | S2 | Vault, AMM |
...
| Security | `amm_bytecode_no_unauthorized_extraction` | `Examples/AmmEndToEnd.lean` | proved | S2 | AMM |
| Deploy | `compileObject_correct` / `bytecode_deploy_correct` | powdr + `Deploy.lean` | Yul ctor proved; EVM args gap | S1 | Token constructor (call-free); Counter has no ctor args |
...
- **Deploy / constructor.** Yul `constructor_correct` (`Proof/Constructor.lean`) uses the
