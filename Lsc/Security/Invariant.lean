...
def RelyAlong [HasCreditValue X] {C : Spec S X E ε} [HasPayable C]
  ...
  | [], _ => True
  | .call c :: tr, w => RelyAlong self rely tr (step self (.call c) w)
  | .env x' :: tr, w => rely w x' ∧ RelyAlong self rely tr { w with ext := x' }

/-- Every call of `C` at `self` preserves `Inv` when it holds in the pre-world. -/
def PreservesInv [HasCreditValue X] (C : Spec S X E ε) [HasPayable C]
    [HasSelfBalance X]
    (self : Address) (Inv : World S X E → Prop) : Prop :=
  ∀ (c : Call C) (w : World S X E), Inv w → Inv (step self (.call c) w)
...
def PreservesInvEnv (_C : Spec S X E ε) (Inv : World S X E → Prop)
    (rely : World S X E → X → Prop) : Prop :=
  ∀ (w : World S X E) (x' : X), Inv w → rely w x' → Inv { w with ext := x' }
...
def PreservesInvAt [HasCreditValue X] (C : Spec S X E ε) [HasPayable C]
  ...
  ∀ (c : Call C) (w : World S X E),
    c.sender ≠ self → Inv w → Inv (step self (.call c) w)
...
def Reachable [HasCreditValue X] {C : Spec S X E ε} [HasPayable C]
    ...
    (rely : World S X E → X → Prop) (self : Address) (w : World S X E) : Prop :=
  ∃ (w₀ : World S X E) (tr : List (Step C)),
    Deployed C w₀ ∧ Wf self tr w₀ ∧ RelyAlong self rely tr w₀ ∧
      w = run self tr w₀
