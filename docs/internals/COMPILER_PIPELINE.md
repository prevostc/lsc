...
- **Reentrancy lock** — `YulDefs.lean` (`reentrancyLockSlot`, `locks`, `lockCheckStmt` / `lockSetPrefix` / `emitLockClear`). Every runtime entry `tload(0)`s; locking functions (`hasExtCall ∧ ¬ isPureRead`) `tstore(0,1)` after the per-function size guard and `tstore(0,0)` on committing `return`/`stop`. Constructor path is unchanged. S2 still takes `ExtOracle.NoReentry` (slice 8A); dropping it is slice 8C.
