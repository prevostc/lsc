...
the selected function's Yul (or `revert(0,0)`). -/
...
/-- `runtimeBlock` is a calldata-size guard plus a selector `switch`. Used by
the dispatcher proofs and by ABI transport (selector uniqueness). -/
