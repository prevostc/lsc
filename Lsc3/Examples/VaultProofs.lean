...
theorem deposit_ok (assets : Amount ASSET assetScale) (h : DepositOk ctx w assets) :
  ...
  simp [IERC20.transferFrom, Tx.run, Tx.erc20TransferFrom_apply, hxfer, Flag.on]
...
theorem withdraw_ok (sharesIn : Amount SHARE shareScale) (h : WithdrawOk ctx w sharesIn) :
  ...
  split_ifs with h0
  · exact (hden h0).elim
  · simp [redeemedAssets, Amount.toNat, Amount.ofNat] at hfit
    simp [hfit]
    simp [IERC20.transfer, Tx.run, Tx.erc20Transfer_apply, hxfer, Flag.on,
      withdrawPost, redeemedAssets, Amount.toNat, Amount.ofNat]
