...
/-- `Ref (IERC20 a)` macro, `asset.impl w`, and `IERC20.Spec`. -/
example (r : Ref (IERC20 testToken)) (w : World Unit ToyExt Unit)
    (_hT : IERC20.Spec
      (r.impl w : IERC20.Impl testToken (World Unit ToyExt Unit) Empty)) : True :=
  trivial
