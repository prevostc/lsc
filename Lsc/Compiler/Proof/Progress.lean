...
theorem op_call_progress {I : Interface} {S X E ε : Type}
    ...
    ∃ V' st' o,
      ExecStmts (yulD calls) (List.replicate n []) (toVEnv tag env) st
        ((emitLetOp tag c {} env.length (.call b m args)).getD {}).stmts V' st' o ∧
      (o = .halt ∨
        (o = .normal ∧ ∃ v, V' = toVEnv tag (v :: env) ∧
  ...
  have hlet0 : ExecStmt (yulD calls) funsN (toVEnv tag env) st
  ...
  obtain ⟨st1, hpre, hpack, hMO, hstatic, hCW⟩ :=
    call_prefix_fwd tag (I := I) (funs := List.replicate (n + 1) []) pre hR hbd hctx henv
  ...
  let V1 : VEnv evm := (extTok tag d, tokv) :: pre
  have hget : VEnv.get V1 (extTok tag d) = some tokv := by
  ...
  let V2 : VEnv evm := (extOk tag d, resp.flag) :: V1
  have hgetOk : VEnv.get V2 (extOk tag d) = some resp.flag := by
    rw [VEnv.get_cons, if_pos rfl]
  obtain ⟨V3, st3, o3, hsuf, hokP, hfailP, _⟩ :=
    call_suffix_fwd tag (funs := List.replicate (n + 1) []) (d := d) (ok := resp.flag)
      (ret := (bindingMethod c b m).2) (assign := some (identV tag d)) hgetOk
  have hsuf' := execStmts_lift_nils (calls := calls) (n := n + 1) hsuf
  have hbody : ExecStmts (yulD calls) funsN1 pre st
      (emitExtCallBody tag c d b m args (some (identV tag d))) V3 st3 o3 := by
    rw [emitExtCallBody_split]
    have hlet' : ExecStmt (yulD calls) funsN1 V1 st1 (callLetOk tag d args) V2 st2 .normal := by
      simpa [callLetOk, V1, V2, st2] using hlet
    have hletS : ExecStmts (yulD calls) funsN1 V1 st1 [callLetOk tag d args] V2 st2 .normal :=
      Step.seqCons hlet' Step.seqNil
    have hpreLet :
        ExecStmts (yulD calls) funsN1 pre st
          (callPrefix tag c d b m args ++ [callLetOk tag d args]) V2 st2 .normal := by
  ...
  rw [hfuns] at hbody
  have hh := hoist_emitExtCallBody tag (calls := calls) c d b m args (some (identV tag d))
  ...
  · subst ho
    ...
    have h1 : extOk tag d ≠ identV tag d := (identV_ne_extOk tag d d).symm
    have h2 : extTok tag d ≠ identV tag d := (identV_ne_extTok tag d d).symm
    have hset : VEnv.set V2 (identV tag d) vU =
        (extOk tag d, resp.flag) :: (extTok tag d, tokv) :: (identV tag d, vU) :: toVEnv tag env := by
      rw [VEnv.set_cons_ne h1, VEnv.set_cons_ne h2]
      simp [VEnv.set, pre]
    have hV3 : V3 = (extOk tag d, resp.flag) :: (extTok tag d, tokv) :: (identV tag d, vU) :: toVEnv tag env := by
      rw [hVeq]
      simp only
      exact hset
...
theorem stmt_call_progress {I : Interface} {S X E ε : Type}
    ...
    ∃ V' st' o,
      ...
      (o = .halt ∨
        (o = .normal ∧ V' = toVEnv tag env ∧ Inv tag Γ c κ ctx w env V' st')) := by
  have ⟨hlenA, hvals⟩ := callWF_arity_of_BindWF hwfCall hbd
  have hn3 : args.length ≤ 3 := hlenA ▸ hbd.harity
  have hR := hinv.rel
  have hctx := hinv.ctxr
  ...
  have hsuf' := execStmts_lift_nils (calls := calls) (n := n + 1) hsuf
  have hbody : ExecStmts (yulD calls) funsN1 pre st
      (emitExtCallBody tag c d b m args none) V3 st3 o3 := by
    rw [emitExtCallBody_split]
    have hlet' : ExecStmt (yulD calls) funsN1 V1 st1 (callLetOk tag d args) V2 st2 .normal := by
      simpa [callLetOk, V1, V2, st2] using hlet
    have hletS : ExecStmts (yulD calls) funsN1 V1 st1 [callLetOk tag d args] V2 st2 .normal :=
  ...
  have hfuns : funsN1 = [] :: funsN := by simp [funsN1, funsN, List.replicate_succ]
  rw [hfuns] at hbody
  have hh := hoist_emitExtCallBody tag (calls := calls) c d b m args none
