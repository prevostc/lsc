...
| Type | File | Status |
|------|------|--------|
| `TokenAmount` | [TokenAmount.md](TokenAmount.md) | Planned core type — only a non-enforcing structure stub exists today (`Lsc/Lib/Linear/TokenAmount.lean`) |
| `Allowance` | [Allowance.md](Allowance.md) | Planned core type — not yet implemented |
| `FlashLoanReceipt` | [FlashLoanReceipt.md](FlashLoanReceipt.md) | Planned core type — not yet implemented |
| `ReentrancyLock` | [ReentrancyLock.md](ReentrancyLock.md) | Planned core type — not yet implemented |
| `Capability` | [Capability.md](Capability.md) | Planned core type — API under review, not yet implemented |
| `OracleReading` | [OracleReading.md](OracleReading.md) | Planned core type — not yet implemented |
| `WithdrawalRequest` | [WithdrawalRequest.md](WithdrawalRequest.md) | Planned core type — not yet implemented |
| `PositionTicket` | [PositionTicket.md](PositionTicket.md) | Planned core type — not yet implemented |
| `TwoPartyAgreement` | [TwoPartyAgreement.md](TwoPartyAgreement.md) | Optional extension (not planned for the core library) — not yet implemented |
...
> **Current status:** AST-level `LinearPermission` and function `permits` fields were removed, and linearity enforcement is not implemented at all today — `Lsc.Lang.Checks.checkLinear` is a no-op stub, and `Lsc/Lib/Linear/TokenAmount.lean` is only a minimal, non-enforcing structure. The sketch below describes the originally-intended design; a replacement capability model is planned to live under `Lib/Linear/` when linearity work resumes.
