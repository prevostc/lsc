...
> **Open design question**: This type's API shape is under review, and it is not yet implemented. The sketch below uses internal `Capability.require` inside the function body. An alternative is a `Capability` function parameter. See [reference/AMM.md](../../reference/AMM.md) for the current sketch. Pick one surface syntax before implementing.
