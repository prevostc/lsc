...
`accrueInterest`'s two theorems are the deliberate exception: they're stated against a fully *concrete* `ContractState` (not just concrete `principal`/`rate` fields on an otherwise-abstract `s`) and proved by `native_decide` instead. `accrueInterest`'s body reads `σ.principal` *twice* (once inside `⸢*⸣?`, once inside the following `+?`); with `s` left abstract, the general-purpose `simp` congruence search used everywhere else in this file does not converge in a reasonable amount of time on that double-read shape (observed: multiple minutes and multiple GB of memory with no result — a worse case of the same 256-bit-numeral risk `deposit`/`setRate`'s proofs are careful to route around via pure-`Nat` side conditions, just triggered by unification search rather than kernel `decide`). Pinning the whole state to a concrete value sidesteps that entirely: `native_decide` compiles the now fully-computable `runS` call to native code and runs it directly, independent of term size. Per the task's explicit guidance, plain `decide` is avoided throughout for the same reason (kernel reduction on 256-bit `BitVec` values is prohibitively slow); every concrete check in this file uses `native_decide` or is phrased as a pure-`Nat` side condition discharged by `norm_num`/`omega`.
