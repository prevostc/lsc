...
2. **Core → Yul.** Two layers in `Lsc/Compiler/Correctness.lean` (`sorry`, not imported by
   ...
   - **S2 (external calls, backward):** dialect `yulD calls := evmWithExternal calls .none .none`.
     ...
     **S2 step (this session):** Backward call lemmas quantify the oracle: `∃ bit, (bit = true → ∀ g, g n = true → run errors ∧ Yul revert) ∧ (bit = false → ∃ v w0, w0.self/log/ncalls equations ∧ ∀ g, g n = false → run = .ok (v, {w0 with faults := g}) ∧ Inv/RX)`. No `Tx.run_faults_congr`. `mapWorldFaults` shares the observational copy so M1 vs `CallFree` matches do not generate distinct `match` auxiliaries (`Nat` vs `RetTy.word.denote`). `core_sim_ext`, S2Frag `toYulFn_correct_ext` (drop `haddr`/`hstab`), and `vault_correct_ext` are **not done**. Resume at `core_sim_ext` in `Proof/CoreExtSim.lean` (after `oracleAgrees` / `callFree_preserves_addr`): `stepOp_ok_ofState` then `execStmts_normal_ofState` for M1-prefix `RX`; `letOp`/`seq` of `.call` compose via `op_sim_call_bwd`/`stmt_sim_call_bwd` + `composeFault`. Vault `deposit`/`withdraw` still M1. `runtimeBlock_correct` stays `sorry` (M3).
