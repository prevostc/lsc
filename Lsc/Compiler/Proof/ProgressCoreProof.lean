  ...
  · have hge4 : 4 ≤ cd.length := Nat.le_of_not_gt hshort
    ...
    | some f =>
      ...
      · have hgeF : 4 + 32 * f.params.length ≤ cd.length := Nat.le_of_not_gt hshortF
        ...
        · by_cases hwrap : f.payable && stA.env.selfBalance.ult stA.env.callvalue
          · have hp : f.payable = true := by
            ...
            have hw : stA.env.selfBalance.ult stA.env.callvalue = true := by
              simp [hp] at hwrap
              exact hwrap
            have hvalH := exec_valueGuardPrefix_wrapHalt_nils (calls := toCalls o)
              (n := 2) (V := []) hp hw
            have hcase : ExecStmts (yulD (toCalls o)) [[], []] [] stA caseBody []
                stRev .halt :=
          ...
          · have hnw : f.payable = false ∨
              ...
              · simp [hpay] at hwrap
                exact Or.inr hwrap
            have hval := exec_valueGuardPrefix_ok_nils (calls := toCalls o) (n := 2)
              (V := []) (valueOk_to_callvalue hctxA hvo) hnw
        ...
        · have hp : f.payable = false := by
          ...
          have hvalH := exec_valueGuardPrefix_valueHalt_nils (calls := toCalls o)
            (n := 2) (V := []) hp hcv
          have hcase : ExecStmts (yulD (toCalls o)) [[], []] [] stA caseBody [] stRev .halt :=
            exec_cons_normal_open hblkF
              (execStmts_append_halt_open (ss2 := lockSetPrefix f ++
