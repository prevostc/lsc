...
/-- External CALL by binding index and method index. Generated by `lsc_schema`. -/
structure ExtSchema (S X E ε : Type) where
  call : Nat → Nat → List Nat → Tx S X E ε Nat

structure ContractSchema (S X E ε : Type) where
  st : StorageSchema S
  ev : EvSchema E
  err : ErrSchema ε
  ext : ExtSchema S X E ε
...
/-- Reads, writes, emits, and external CALLs (`binding`, `method` indices) of a Core term. -/
...
/-- Default `ext.call`: every CALL fails. Used when the contract has no `Ext`. -/
def ExtSchema.noneCall : ExtSchema S X E ε where
  call := fun _ _ _ => fun _ _ => .error .callFailed
