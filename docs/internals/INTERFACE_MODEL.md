...
`α : Abs I.Ghost` (`ofState` / `ofWorld` at a callee address). For every successful call from `self` to `addr`: decode matches
...
`RelyAlong`: between two of our calls, each binding's ghost evolves under `I.Rely self`.
...
## Core and `toYulFn_correct_*`

Core gains exactly `Op.call b m args` and `Stmt.call b m args`. `ContractSchema.ext` supplies
`call : Nat → Nat → List Nat → Tx`. Lowering: `YUL_TARGET.md`. Fault-oracle shape:
`PROOF_CHAIN.md`. TCB: `TRUSTED_COMPUTING_BASE.md`. Call-free Core → Yul is
