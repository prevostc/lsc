...
/-- Vault slots: `totalAssets=0`, `totalShares=1`, `shares=2` (map), `paused=3`, `owner=4`. -/
def mkStorage (ta ts paused owner : Nat) : Storage :=
  fun k =>
    if k = 0 then ta
    else if k = 1 then ts
...
#eval! do
  ...
  | .ok code => do
    ...
    IO.println s!"previewDeposit 100 on empty: {formatOutcome (exec code (packCall previewDepSel [100]) st0)}"
    match exec code (packCall depositSel [100]) st0 who with
    | .ret minted s1 => do
      IO.println s!"deposit 100 → minted {minted}, ta={s1 0} ts={s1 1}"
      IO.println s!"previewRedeem 100: {formatOutcome (exec code (packCall previewRedSel [100]) s1)}"
      match exec code (packCall withdrawSel [40]) s1 who with
      | .ret out s2 => do
        IO.println s!"withdraw 40 → {out} assets, ta={s2 0} ts={s2 1}"
        match exec code (packCall pauseSel) s2 owner with
        | .stop s3 => do
          IO.println s!"pause by owner: paused={s3 3}"
          IO.println s!"deposit while paused: {formatOutcome (exec code (packCall depositSel [10]) s3 who)}"
          IO.println s!"pause by non-owner: {formatOutcome (exec code (packCall pauseSel) s2 who)}"
          match execState code (packCall unpauseSel) s3 owner with
          | some (Halt.stop, s4) =>
