...
way through `derive_contract_def` to bytecode with a genuine `RETURN` (not just `STOP`). -/
...
derive_contract_dsl VStorage VError VEvent
...
-- `ifReturnBody`'s direct `Stmt.evalView` unit tests above already exercise `if`/`return`
...
derive_contract_def "V" VStorage VError VEvent
...
-- The compiled bytecode is real (`derive_contract_def`'s auto-generated `bytecodeHex`, using
