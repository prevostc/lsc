...
/-!
# LSC v3 — contract assembly: the `ContractDef` data model

`lsc_contract C` (in `Lsc3.Reify`, Phase B) collects the entrypoints of a contract — the
definitions tagged `@[lsc_tx]`, `@[lsc_view]`, `@[lsc_ctor]` under namespace `C` — reifies each
one, and produces `C.contract : ContractDef`. Everything downstream is a Lean function of that
value: ABI JSON, selectors, the dispatcher, codegen, and the `bytecode_ok` certificate.
