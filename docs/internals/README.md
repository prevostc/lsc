...
Token, Vault, and Cpamm anti-exploit theorems are proved at `Tx.run` / trace
level (`token_no_unauthorized_extraction`, `vault_no_unauthorized_extraction`,
`cpamm_no_unauthorized_extraction`, and the matching solvency theorems).
Bytecode transport is `transport_claim_ext` / `transport_exists_claim_ext`
(`Lsc/Compiler/TransportTheorems.lean`). Counter is compiler-level only.
...
- `INTERFACE_MODEL.md` — `I.Ref` / `I.Impl` / `I.Spec`, `Oracle`, `Tx.call`.
- `PROOF_CHAIN.md` — links from a `Tx` theorem to bytecode.
...
- `AMM_EXAMPLE.md` — identifier-dense Cpamm status (walkthrough: `docs/guide/AMM.md`).
