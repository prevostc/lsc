...
`read`/`write`, `require`, and `emit`. Certificates (`denoteAWord` / `denoteAUnit`)
must close by `rfl`. `lsc_contract` is omitted: `worldAfter_core_eq` is stated
with `Core.denote` (Nat), and there is not yet a lemma that those worlds agree
with the Amount denotations.
-/

open Lsc Lsc.Syntax Stdlib
