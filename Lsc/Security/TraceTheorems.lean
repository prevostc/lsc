...
/-- Unpacking `ofCtx` recovers the original context at `ctx.self`. -/
...
@[simp] theorem Call.ofCtx_toCtx (c : Call C) (self : Address) :
    Call.ofCtx (c.toCtx self) c.fn c.args = c :=
  Proof.Call.ofCtx_toCtx c self

/-- `toCtx` writes the callee address into `Ctx.self`. -/
@[simp] theorem Call.toCtx_self (c : Call C) (self : Address) :
    (c.toCtx self).self = self :=
  rfl

/-- `toCtx` preserves the sender. -/
@[simp] theorem Call.toCtx_value (c : Call C) (self : Address) :
    (c.toCtx self).value = c.value :=
  rfl
...
theorem run_append [HasCreditValue X] [HasPayable C] [HasSelfBalance X]
    ...
    (w : World S X E) :
    run self (tr₁ ++ tr₂) w = run self tr₂ (run self tr₁ w) :=
  Proof.run_append self tr₁ tr₂ w
...
theorem step_eq_worldAfter_of_credit_id [HasCreditValue X] [HasPayable C]
  ...
  Proof.step_eq_worldAfter_of_credit_id self c w hvo h hnw
...
theorem step_eq_worldAfter_of_not_payable [HasCreditValue X] [HasPayable C]
    ...
    (hp : C.payable c.fn = false) (hv : c.value = 0) :
    step self (.call c) w = worldAfter (C.exec c.fn c.args) (c.toCtx self) w :=
  Proof.step_eq_worldAfter_of_not_payable self c w hp hv
