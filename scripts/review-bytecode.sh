...
#
# Usage: scripts/review-bytecode.sh [Counter|Token|Vault|Cpamm|WNative|all]
set -euo pipefail
cd "$(dirname "$0")/.."
...
case "$TARGET" in
  Counter|Token|Vault|Cpamm|WNative) review_one "$TARGET" ;;
  all)
    for c in "${CONTRACTS[@]}"; do review_one "$c"; done
    ;;
  *)
    echo "Usage: $0 [Counter|Token|Vault|Cpamm|WNative|all]" >&2
    exit 1
    ;;
esac
